...
import { phoenixGql, phoenixUrl } from './phoenix'
import { subscribeToSearchTransport, type SearchEvent } from './search-transport'
  ...
  const sendMessage = useCallback(
    async (query: string) => {
      ...
      try {
        ...
        const unsubscribe = subscribeToSearchTransport(
          sessionId,
          chatId,
          msgId,
          phoenixUrl,
          {
            ...
            onError: (error: string) => {
              console.warn('[search transport error]', error)
              // Transport failover is handled internally by the orchestrator.
              // If all transports fail, the orchestrator emits error events
              // which are handled by the 'error' case above.
            },

            onComplete: () => {
              // Transport ended cleanly or was aborted
            },
          },
        )

        unsubRef.current = unsubscribe.unsubscribe
      } catch (err: any) {
        updateMsg(msgId, m => ({
          ...m,
          status: 'error',
    ...
    [loading, mode, messages, updateMsg]
  )
